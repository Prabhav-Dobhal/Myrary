    ...
    try {
        ...
        for (const bookID of selectedbooks) {
            if (mongoose.Types.ObjectId.isValid(bookID)) {
                //convert bookID to proper mongoose objectId
                ...
                return res.send(`Invalid Book ID ${bookID}`);
            }
        }
        ...
        res.render("issuesuccess");
    } catch (e) {
        res.send("Error:" + e);
    }
});

app.get("/issuesuccess",(req,res)=>{
    res.render("issuesuccess");
})
...
app.post("/returnbook",async(req,res)=>{
    try{
        ...
        const user= await Issuedbooks.findOne({phoneno:phoneno});

        if(user){
            if((username===user.username)){
                const books=user.books;
                for(const booksit of books){
                        const increment_book = await Book.findOne({ book: booksit });
                        if (increment_book) {
                            increment_book.quantity++;
                            await increment_book.save();
                        } else {
                            return res.send("Books find error!!");
                        }
                }
            } 
            else{
            ...
            //delete user
            const del= await Issuedbooks.deleteOne({ phoneno: phoneno });
            if(!del){
                res.send("ERROR: Issuer record not deleted !");
            }
            res.render("changesuccessful");
        }
        else{
...
app.get("/changesuccessful",(req,res)=>{
    res.render("changesuccessful");
})
